...
def _differences(left: Any, right: Any, path: str = "$", *, limit: int = MAX_REPORTED_DIFFERENCES) -> list[str]:
    ...
    def walk(a: Any, b: Any, current: str) -> None:
        ...
        if type(a) is not type(b):
            found.append(f"{current}: type {type(a).__name__} != {type(b).__name__}")
            return
...
def _decision_summary(chain: tuple[dict[str, Any], dict[str, Any], dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in plan:
        ...
        if first_action is None and action not in {"geen_actie", "unknown", "none", "None"}:
            first_action = {
                "time": row.get("time"),
                "action": row.get("action"),
                "charge_from_grid_kwh": row.get("charge_from_grid_kwh"),
                "charge_from_grid_safety_kwh": row.get("charge_from_grid_safety_kwh"),
                "charge_from_grid_trade_kwh": row.get("charge_from_grid_trade_kwh"),
    ...
    return {
        ...
        "additional_grid_charge_kwh": need.get("energy_need_additional_grid_charge_kwh"),
...
def compare_frozen_live_snapshot(
    ...
    if not isinstance(input_result, dict):
        ...
        if input_result.get("valid") is not True:
            blockers.append("input_invalid")
        if not isinstance(input_result.get("rows"), list) or len(input_result.get("rows") or []) != 72:
