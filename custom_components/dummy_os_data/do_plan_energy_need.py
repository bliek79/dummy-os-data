...
def build_do_plan_energy_need(
    *,
    ...
    Battery capacity and hardware minimum are fixed alpha76 invariants during
    parity migration.  Passing different values is rejected rather than silently
    changing the copied EMS policy.
    """
    if abs(float(battery_capacity_kwh) - BATTERY_CAPACITY_KWH) > 1e-9:
        return {
            "status": "blocked", "valid": False,
            "reason": "alpha76_battery_capacity_mismatch",
            "blockers": ["alpha76_battery_capacity_mismatch"],
    ...
    if abs(float(min_soc_percent) - MIN_SOC_PERCENT) > 1e-9:
        return {
            "status": "blocked", "valid": False,
            "reason": "alpha76_min_soc_mismatch",
            "blockers": ["alpha76_min_soc_mismatch"],
            "ems_policy_source": "0.0.1-alpha.76",
            "shadow_only": True, "active_use_permitted": False,
            "physical_execution_authority": False,
        }
