...
class FakeServices:
    ...
    async def async_call(self, domain, service, data, target=None, blocking=True):
        ...
        stable_at = datetime.now(timezone.utc) - timedelta(seconds=120)
        if domain == "select" and eid == "select.mode":
            option = data["option"]; self.hass.states.values[eid] = FakeState(option, stable_at); self.coordinator.data["operating_mode"] = option
        elif domain == "select" and eid == "select.direction":
            option = data["option"]; self.hass.states.values[eid] = FakeState(option, stable_at); self.coordinator.data["action_direction"] = option
        elif domain == "number" and eid == "number.power":
            value = data["value"]; self.hass.states.values[eid] = FakeState(value, stable_at); self.coordinator.data["power_setpoint_w"] = value
class FakeHass:
    def __init__(self, states): self.states = FakeStates(states); self.services = None
    def async_create_task(self, coro, *args):
        # Execution tests assert state transitions explicitly. Do not let the
        # background max-runtime task race the test; close its coroutine cleanly.
        coro.close(); return FakeTask()
...
async def test_automatic_execution_exact_safe_order_and_safe_stop(monkeypatch):
    ...
    stable_at = datetime.now(timezone.utc) - timedelta(seconds=120)
    states = {"select.mode": FakeState("self_consumption", stable_at), "select.direction": FakeState("charge", stable_at), "number.power": FakeState(0, stable_at)}
...
async def test_execution_monitor_emergency_stops_on_direction_change(monkeypatch):
    ...
    monkeypatch.setattr(execution_module.asyncio, "sleep", no_sleep)
    stable_at = datetime.now(timezone.utc) - timedelta(seconds=120)
    states = {"select.mode": FakeState("third_party_control", stable_at), "select.direction": FakeState("discharge", stable_at), "number.power": FakeState(800, stable_at)}
    store = MemoryPlanStore(); store.plans[1] = automatic_plan(start=BASE)
